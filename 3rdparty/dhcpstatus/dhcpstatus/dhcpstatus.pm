...
sub parse_conf_sym {
   ...
   while (@$symref) {
      if ($$symref[0] eq "include") {			# found a subnet, so
         my @include_conf_sym = &get_symbols($$symref[1]);
         push(@subnet, &parse_conf_sym(\@include_conf_sym, %parms));	# process it and add it
         #$parms{"comment"} = $$symref[1];		# it to describe our
